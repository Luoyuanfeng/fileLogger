...
func (f *FileLogger) initLoggerByDaily() {
	...
	logFile := joinFilePath(f.fileDir, f.fileName)
	logFileWithSuffix := fileAddSuffix(logFile)
	if !f.isMustSplit() {
		if !isExist(f.fileDir) {
			os.Mkdir(f.fileDir, 0755)
		}
		f.logFile, _ = os.OpenFile(logFileWithSuffix, os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
		f.lg = log.New(f.logFile, f.prefix, log.LstdFlags|log.Lmicroseconds)
	} else {
		f.split()
	}
...
func (f *FileLogger) split() {
	...
	case SplitType_Daily:
		logFileBak := logFile + "-" + f.date.Format(DATEFORMAT) + ".log"
		logFileWithSuffix := fileAddSuffix(logFile)
		if !isExist(logFileBak) && f.isMustSplit() {
			...
			err := os.Rename(logFileWithSuffix, logFileBak)
			if err != nil {
			...
			t, _ := time.Parse(DATEFORMAT, time.Now().Format(DATEFORMAT))
			f.date = &t
			f.logFile, _ = os.Create(logFileWithSuffix)
			f.lg = log.New(f.logFile, f.prefix, log.LstdFlags|log.Lmicroseconds)
		}
	}
}
...
func fileAddSuffix(fileBase string) string {
	return fileBase + ".log"
}
